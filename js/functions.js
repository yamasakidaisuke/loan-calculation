...
function PMT() {
	...
	var np = parseFloat(document.loan.np.value);//返済期間（年数）
...
return Math.floor((((pv - (bonus * bonusN * np)) - deposit) * (ir / 100 /12) * (Math.pow(1 + (ir / 100 /12), np*12))) / (Math.pow(1 + (ir / 100 / 12), np*12) -1)).toLocaleString();
}
...
function PMT2() {
	...
	var bonus = parseFloat(document.loan.bonus.value);//ボーナス予定返済額（1回分）
	var bonusN = parseFloat(document.loan.bonusN.value);//ボーナス予定返済回数（n）
	var ir = parseFloat(document.loan.ir.value);//金利（年）
...
return Math.floor(((bonus * bonusN * np) *(ir/100/bonusN)*(Math.pow ( 1 +(ir/100/bonusN),(np*bonusN)))) / ((Math.pow(1+(ir/100/bonusN),(np*bonusN)))-1)).toLocaleString();
}